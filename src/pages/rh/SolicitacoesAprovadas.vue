<template>
  <div class="flex flex-col space-y-2">
    ...
    <div v-else class="bg-white rounded-md w-full h-screen py-10">
      <h1 class="text-center text-3xl font-bold text-orange-400 m-auto animate-pulse">Nenhuma solicitação aprovada</h1>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async fetchApprovedRequests() {
      this.loading = true;
      try {
        const status = ["DEF", "VGE"]
        const response = await api.get(`/api/v1/solicitacao/?status__in=${status}`);
